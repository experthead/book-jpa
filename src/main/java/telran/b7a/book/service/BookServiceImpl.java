...
//				.collect(Collectors.toList());
		Author author = authorRepository.findById(authorName).orElseThrow(() -> new EntityNotFoundException()); //не нужен transactional
...
//				.map(p -> modelMapper.map(p, BookDto.class))
//				.collect(Collectors.toList());	
		Publisher puslisher = publisherRepository.findById(publisherName).orElseThrow(() -> new EntityNotFoundException());
		return puslisher.getBooks().stream()
				.map(b -> modelMapper.map(b, BookDto.class))
				.collect(Collectors.toList());
	}
	...
	public Iterable<AuthorDto> findBooksAuthors(String isbn) {
		Book books = bookRepository.findById(isbn).orElseThrow(() -> new EntityNotFoundException());
	...
	@Override
	@Transactional(readOnly = true)
	public Iterable<String> findPublishersAuthor(String authorName) {
...
//		bookRepository.findByAuthors_Name(authorName)
//				.forEach(b -> bookRepository.delete(b));  //CASCADE.ALL in Author Entity
